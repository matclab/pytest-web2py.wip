...
from pytest_web2py.unit import *
...
def describe_register():
    ...
    def login_fail_until_registration_verified(w, register_and_login, gae,
        ...
        r = form_post(w, 'user/login', login_data)
    ...
    def fail_when_erroneous_second_password(w, user_data):
        r = form_post(w, 'user/register', user_data)
...
def describe_index():
    def index_does_display(w, db, controller):
        html = call(w, 'index', controller=controller, render=True).html
        assert '>Home</a>' in html
        assert 'Sign Up' in html
        assert 'Log In' in html

    def index_does_display_when_logged(w, db, controller, register_and_login):
        html = call(w, 'index', controller=controller, render=True).html
        assert '>Home</a>' in html
